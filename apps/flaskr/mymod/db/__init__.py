...
from apps.flaskr.mymod.db import connect
from apps.flaskr.mymod.db.host import HostFile, host
from apps.flaskr.mymod.log import LOGGER
from apps.flaskr.mymod.system import system
...
class SyncHost:
    ...
    def _refresh(
        ...
        # HOST -> df
        LOGGER.info(
            f"Syncing {h.file_name}.{h.lib_name} ({where_sqlite3}) ..."
        )
    ...
    def refresh_all(
        ...
        system.last_refresh_date = \
            datetime.now().strftime(r"%Y/%m/%d %H時%M分%S秒")
