...
def refresh_all(
    ...
    try:
        ...
        file_ins.TEMP_URI2.refresh(where=f"伝票日付>={yymmdd_host}")
        file_ins.TOTAL_SYUKA.refresh(where=f"伝票日付>={yymmdd_host}")
        file_ins.TOTAL_SIIRE.refresh(where=f"伝票日付>={yymmdd_host}")
        if contain_master:
            file_ins.EIGYO_MASTER.refresh()
            file_ins.KOUJO_MASTER.refresh()
            file_ins.SOKCD_MASTER.refresh()
            file_ins.TOKCD_MASTER.refresh()
            file_ins.TEHAI_MASTER.refresh()
            #file_ins.KEN_MASTER.refresh()
            file_ins.BUHIN_MASTER.refresh()
            file_ins.SEIHIN_MASTER.refresh()
            file_ins.HINMOKU_MASTER.refresh()
